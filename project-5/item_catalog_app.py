...
def showBooksJSON(bookbin_id):
	...
	books = session.query(Book).filter_by(bookbin_id = bookbin_id).order_by(asc(Book.title)).all()
...
def newBookbin():
	...
	if request.method == 'POST':
		newBookbin = Bookbin(name = request.form['name'], reader_id = login_session['reader_id'])
...
def deleteBookbin(bookbin_id):
	...
	if request.method == 'POST':
		...
		return render_template('deleteBookbin.html', bookbin=bookbinToDelete)
...
def showBooks(bookbin_id):
    ...
    books = session.query(Book).filter_by(bookbin_id = bookbin_id).order_by(asc(Book.title)).all()
    if 'readername' not in login_session or creator.id != login_session['reader_id']:
    	return render_template('publicBooks.html', books = books, bookbin = bookbin, creator=creator.name)
    else:
    	return render_template('books.html', books=books, bookbin=bookbin, creator=creator.name)
...
def newBook(bookbin_id):
	...
	if request.method == 'POST':
	  newBook = Book(title = request.form['title'], author = request.form['author'], pub_year = request.form['pub_year'], description = request.form['description'], genre = request.form['genre'], bookbin_id = bookbin_id, reader_id=login_session['reader_id'])
...
#Edit a book
@app.route('/bookbin/<int:bookbin_id>/books/<int:book_id>/edit', methods=['GET','POST'])
def editBook(bookbin_id, book_id):
	...
	if not editedBook:
		return redirect(url_for('showBooks', bookbin_id = bookbin_id, creator=creator.name))
	...
	if request.method == 'POST':
		...
		if request.form['pub_year']:
			editBook.pub_year = request.form['pub_year']
			session.add(editedBook)
			session.commit() 
		flash('Book Successfully Edited')
		return redirect(url_for('showBooks', bookbin_id = bookbin_id, creator=creator.name))
	else:
	    return render_template('editBook.html', bookbin= bookbin, book = editedBook)


#Delete a book
@app.route('/bookbin/<int:bookbin_id>/books/<int:book_id>/delete', methods = ['GET','POST'])
def deleteBook(bookbin_id, book_id):
	...
	if request.method == 'POST':
		...
		return render_template('deleteBook.html', bookbin_id = bookbin_id, book=bookToDelete)
...
def createReader(login_session):
	newReader = Reader(name = login_session['readername'], email = login_session['email'])
	session.add(newReader)
	session.commit()
	reader = session.query(Reader).filter_by(email = login_session['email']).one()
	return reader.id
